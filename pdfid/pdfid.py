...
def C2BIP3(string):
    """
    Converts a string to a bytes object in Python 3, or returns the string as-is in Python 2.

    Args:
        string (str): The input string to convert.

    Returns:
        bytes or str: The input as bytes (Python 3) or string (Python 2).
    """
...
def CreateZipFileObject(arg1, arg2):
    """
    Creates and returns a ZipFile or AESZipFile object for reading ZIP archives.

    Args:
        arg1 (str): The filename of the ZIP archive.
        arg2 (str): The mode to open the archive (e.g., "r" for read).

    Returns:
        ZipFile or AESZipFile: An object for interacting with the ZIP archive.
    """
...
class cBinaryFile:
    """
    Abstraction for opening and reading from various file sources (local, URL, ZIP, stdin, or in-memory).

    Methods:
        byte(): Reads and returns the next byte as an integer, or None at EOF.
        bytes(size): Reads and returns a list of the next 'size' bytes.
        unget(byte): Pushes a byte back to the buffer to be read again.
        ungets(bytes): Pushes a list of bytes back to the buffer in reverse order.
    """

    def __init__(self, file, data=None):
        """
        Initializes the binary file object.

        Args:
            file (str): Path, URL, or ZIP file to open, or empty string for stdin.
            data (bytes, optional): If given, uses this as the file content.
        """
...
class cPDFDate:
    """
    State machine for parsing PDF date strings from a stream of characters.

    Methods:
        parse(char): Feeds a character to the parser. Returns the parsed date string if complete, otherwise None.
    """
...
def fEntropy(countByte, countTotal):
    """
    Calculates the partial entropy for a byte value, given its count and total count.

    Args:
        countByte (int): The count of a specific byte value.
        countTotal (int): The total count of all byte values.

    Returns:
        float: The entropy contribution of this byte value.
    """
...
class cEntropy:
    """
    Tracks byte frequency distributions for a PDF file and calculates entropy for the entire file,
    for streams, and for non-stream data.

    Methods:
        add(byte, insideStream): Adds a byte to the frequency count, optionally marking it as inside a stream.
        removeInsideStream(byte): Removes a byte from the stream frequency count.
        calc(): Returns entropy statistics as a tuple.
    """
...
class cPDFEOF:
    """
    State machine for detecting and counting '%%EOF' markers in a PDF file,
    which indicate the end of the file.

    Methods:
        parse(char): Feeds a character to the parser, updating EOF counters.
    """
...
def FindPDFHeaderRelaxed(oBinaryFile):
    """
    Searches the first 1024 bytes of a file for a '%PDF' header and returns the header bytes and string.

    Args:
        oBinaryFile (cBinaryFile): The binary file object to search.

    Returns:
        tuple: (header_bytes, header_string) or ([], None) if not found.
    """
...
def Hexcode2String(char):
    """
    Converts a byte or character to a hex string if it's an integer, otherwise returns the character.

    Args:
        char (int or str): The byte or character.

    Returns:
        str: Hex string or character.
    """
...
def SwapCase(char):
    """
    Swaps the case of a character or integer byte value.

    Args:
        char (int or str): The character or byte.

    Returns:
        int or str: Case-swapped value.
    """
...
def HexcodeName2String(hexcodeName):
    """
    Converts a sequence of hex codes or characters to a string.

    Args:
        hexcodeName (iterable): Iterable of bytes or characters.
...
def SwapName(wordExact):
    """
    Swaps the case for each character in a word.

    Args:
        wordExact (iterable): Iterable of characters or bytes.

    Returns:
        map: Iterator of case-swapped characters.
    """
...
def UpdateWords(
    ...
    """
    Updates keyword statistics and handles special cases (stream, endstream, suspicious names).

    Args:
        word (str): Current word being parsed.
        wordExact (list): List of exact characters for the word.
        slash (str): Slash prefix for PDF names.
        words (dict): Dictionary tracking keyword counts.
        hexcode (bool): Whether the word contains hex codes.
        allNames (bool): Whether to track all names.
        lastName (str): Last PDF name seen.
        insideStream (bool): Whether currently inside a stream.
        oEntropy (cEntropy): Entropy tracker object.
        fOut (file): Output file for disarmed PDFs.

    Returns:
        tuple: Updated (word, wordExact, hexcode, lastName, insideStream).
    """
    if word != "":
...
class cCVE_2009_3459:
    """
    Detector for the CVE-2009-3459 vulnerability in PDFs,
    which is triggered by an excessive number of colors.

    Methods:
        Check(lastName, word): Increments count if the vulnerability is detected.
    """
...
def XMLAddAttribute(xmlDoc, name, value=None):
    """
    Adds an attribute to the root element of an XML document.

    Args:
        xmlDoc (xml.dom.minidom.Document): The XML document.
        name (str): Attribute name.
        value (str, optional): Attribute value.

    Returns:
        xml.dom.minidom.Attr: The created attribute node.
    """
...
def GetScriptPath():
    """
    Returns the directory path of the running script or executable.

    Returns:
        str: The script or executable directory.
    """
...
def PDFiD(file, allNames=False, extraData=False, disarm=False, force=False, data=None):
    """
    Scans a PDF file for suspicious elements and metadata, returning an XML document with statistics.
    ...
    Args:
        file (str): Path to the PDF file to scan.
        allNames (bool): If True, track all PDF names.
        extraData (bool): If True, collect additional data (dates, entropy, EOF markers).
        disarm (bool): If True, output a disarmed version of the PDF.
        force (bool): If True, continue scanning even if the file doesn't appear to be a PDF.
        data (bytes, optional): If given, use this data instead of reading from file.

    Returns:
        xml.dom.minidom.Document: An XML document containing scan results, statistics, and metadata.
        If you need the rest of the file or want comments in a different format (inline or above each function), let me know!

        Related
        What information does each function process and produce in this file
        How do the functions interact with each other to perform the overall task
        What parameters do the functions require and what are their data types
        Are there any specific data structures or return types used by these functions
        How can I modify or extend these functions based on their current implementation
    """
